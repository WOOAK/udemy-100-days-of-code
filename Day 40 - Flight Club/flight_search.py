...
class FlightSearch:
    ...
    def cheapest_flight(self,code,start_date,end_date):
        self.flight_search_parms = {
            ...
            # initial try with 0 direct flight
            "max_stopovers": 0,
        ...
        # try direct flight, if there is no direct flight, go to max stop 4
        try:
            ...
            # try indirect flight
            try:
                self.flight_search_parms["max_stopovers"] = 1
                print(self.flight_search_parms)
                print(code)
                self.flight_data = (requests.get(url=TEQUILA2_ENDPOINT, params=self.flight_search_parms, headers=teq_header).json())
                price = self.flight_data["data"][0]["conversion"]["GBP"]
                departure_airport = self.flight_data["data"][0]["route"][0]["flyFrom"]
                arrival_airport = self.flight_data["data"][0]["route"][1]["flyTo"]
                link = self.flight_data["data"][0]["deep_link"]
                go_date = self.flight_data["data"][0]["route"][0]["local_departure"].split("T")[0]
                return_date = self.flight_data["data"][0]["route"][2]["local_departure"].split("T")[0]
                via_city = self.flight_data["data"][0]["route"][1]["cityTo"]
                print(price)
                print(f"test:{via_city}")
                flight_data = FlightData(price, departure_airport,arrival_airport,link,go_date,return_date,stop_overs=1,via_city=via_city)
                print(via_city)
                return flight_data
            except IndexError:
